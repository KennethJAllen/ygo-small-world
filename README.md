...
More specifically, the bridge score for any particular bridge $b$ is the number of pairs of cards $(i,j)$ such that $i$ is searchable starting from $j$ via Small World divided by $(n+1)^2$, where $i$ and $j$ are cards in the deck or the bridge.
...
If there are $m$ possible bridges $b$, then calculating the Small World bridge score for every bridge with this approach only takes $O(n^3 + mn^2)$ multiplications. This is much better than the naive approach of performing the full matrix multiplication for $M_b^2$ for each bridge $b$, which would take $O(mn^3)$ multiplications.
