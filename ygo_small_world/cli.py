...
def cli():
    ...
    Default output path is ./output in the working directory.

    Args:
    - ydk_file (str): Path to a YDK file; only main-deck monsters are analyzed.

    Optional Args:
    - --update_data (bool): Updates the local card database.
    ...
    parser.add_argument(
        "-u", "--update_data",
        action="store_true",
        help="Update the local card database."
    )
    parser.add_argument(
        "-o", "--output",
        type=str,
