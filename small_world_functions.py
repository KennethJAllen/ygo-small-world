...
def matrix_to_graph_image(connection_matrix, card_images):
    ...
    for n in G:
        ...
        a.axis('off')
    ax.axis('off')
    plt.savefig('images\small-wolrd-graph.png')
    plt.show()
...
def matrix_to_image(connection_matrix, card_images, squared=False, highlighted_columns=[]):
    ...
    ax.axis('off')
    if squared==False:
        plt.savefig('images\small-world-matrix.png', dpi=128.5, bbox_inches='tight')
    else:
        plt.savefig('images\small-world-matrix-squared.png', dpi=128.5, bbox_inches='tight')
    plt.show()
